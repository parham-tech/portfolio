 ...
 *  - cacheTtlMs (number) default 60_000 (cache coin list 60s)
 *  - chartCacheTtlMs (number) default 300_000 (cache charts 5min)
 *  - theme (string) optional (used to apply class names from parent if provided)
 *  - themeClasses (object) optional mapping theme->className
 *
 * Notes:
 *  - Uses CoinGecko public endpoints (no API key).
 *  - Uses in-memory + sessionStorage caching to reduce API calls.
 *  - Cancels inflight fetches using AbortController.
 */

// -------------------- config --------------------
const COINGECKO_BASE = "https://api.coingecko.com/api/v3";
const DEFAULT_PER_PAGE = 40;
const DEFAULT_CACHE_TTL = 60_000; // 60s for coin list
const DEFAULT_CHART_CACHE_TTL = 300_000; // 5min for charts
const WATCHLIST_KEY = "cg_watchlist_v3";

// -------------------- helpers --------------------
const now = () => Date.now();

const cachedFetch = async (url, { ttl = DEFAULT_CACHE_TTL, useSession = true } = {}) => {
  // sessionStorage key
  const key = `cache_v1::${url}`;
  try {
...
export default function CryptoDashboard({
  ...
  // refs for cancellation & rate-limiting
  ...
  // fetch coin list (with simple rate-limit + caching + abort)
  useEffect(() => {
    let cancelled = false;
    const loadCoins = async () => {
      ...
      // avoid hammering CoinGecko: if last fetch < 5s, wait a bit (soft rate-limit)
      const nowTs = now();
      if (nowTs - lastCoinsFetchRef.current < 4000) {
  ...
  // fetch chart for selected (use cached sessionStorage if available)
  useEffect(() => {
    if (!selected) {
      setChartData(null);
      return;
    ...
    const loadChart = async () => {
      ...
      setChartError(null);
      try {
        const url = `${COINGECKO_BASE}/coins/${selected.id}/market_chart?vs_currency=usd&days=${days}&interval=hourly`;
        const res = await fetch(url, { signal });
        if (!res.ok) throw new Error(`${res.status} ${res.statusText}`);
        ...
        <div className="flex-1 bg-gray-800/40 rounded p-4">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
            ...
            <div className="flex items-center gap-2">
              <div className="text-xs text-gray-300 hidden md:block">Sort:</div>
              <select
                aria-label="Sort key"
              ...
              <button aria-label="Toggle sort direction" onClick={() => setSortBy((s) => ({ ...s, dir: s.dir === "asc" ? "desc" : "asc" }))} className="px-2 py-2 rounded bg-gray-700 text-sm">
                {sortBy.dir === "asc" ? "↑" : "↓"}
              </button>

              <button onClick={handleExportCsv} className="px-3 py-2 rounded bg-indigo-600 text-sm" aria-label="Export CSV">
                Export CSV
              </button>
            </div>
          </div>
            ...
            <div className="space-y-2 max-h-[580px] overflow-auto mt-3">
              {paged.map((c) => (
                ...
                >
                  ...
                  <div className={`col-span-2 text-right text-sm ${c.price_change_percentage_24h >= 0 ? "text-green-400" : "text-red-400"}`}>
                  ...
                  <div className="col-span-12 flex justify-end gap-2 mt-2 md:mt-0">
                    <button
                      ...
                      className={`px-2 py-1 text-xs rounded ${watchlist.includes(c.id) ? "bg-yellow-400 text-black" : "bg-gray-700 text-white"}`}
                      aria-label={watchlist.includes(c.id) ? `Remove ${c.name} from watchlist` : `Add ${c.name} to watchlist`}
                    >
                      {watchlist.includes(c.id) ? "★ Watch" : "☆ Watch"}
                    </button>
                    <button
                      onClick={(e) => { e.stopPropagation(); setSelected(c); }}
                      className="px-2 py-1 text-xs rounded bg-indigo-600"
                      aria-label={`View ${c.name} chart`}
                    >
                      View
                    </button>
                  </div>
            ...
            <>
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <div className={`text-xs ${selected.price_change_percentage_24h >= 0 ? "text-green-400" : "text-red-400"}`}>
              ...
              <div className="flex gap-2 items-center">
                {[1,7,30,90,180,365].map((d) => (
                  <button
                    key={d}
                    onClick={() => setDays(d)}
                    className={`px-2 py-1 rounded text-xs ${days === d ? "bg-indigo-600" : "bg-gray-800"}`}
                    aria-pressed={days === d}
                  >
                    {d}d
                  </button>
